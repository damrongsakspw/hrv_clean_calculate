...
for j in range(1,17):
    if j != 3:
        ...
        # Read the Excel file
        df = pd.read_excel(excel_file_path,sheet_name='Pre-Post Test')
        for i in range(0,6):
            print(i)
            selected_cell = df.iloc[i, 9]  # เลือกเซลล์ในแถวแรกและคอลัมน์แรก
            ...
            output_file = f'./pase/{j}-{i}parsed_data.xlsx'
