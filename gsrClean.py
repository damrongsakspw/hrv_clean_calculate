...
for j in range(1, 17):
    if j != 3:
        ...
        # Read the Excel file
        sheet_name = 'Pre-Post Test'
        df = pd.read_excel(excel_file_path, sheet_name=sheet_name)
        # Get the number of rows
        num_rows = df.shape[0]

        for i in range(num_rows):
            selected_cell = df.iloc[i, 8]  # Select the cell in the 11th column
            date_time = df.iloc[i, 10]
            selected_gsr = df.iloc[i, 7]  # Select the cell in the 10th column
            ...
            # Define new Excel file name
            output_file = f'./graph_pase/{j}-{i}parsed_data_graph.xlsx'
